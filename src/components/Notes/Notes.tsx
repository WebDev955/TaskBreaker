    ...
    <>
      <div className={style.mainNotesWrapper}>
        <div className={style.noteBookHeader}>
          <h1>NoteBook</h1>
          <p>
            Got an idea that popped into your head? <br /> Take note of it.
          </p>
          ...
          {openNote === true && (
            <div className={style.newNoteDiv}>
              <button onClick={() => saveNote(noteDraft, noteTitle)}>
                Save Notes
              </button>
              <input
                type="text"
              ...
              />
              <textarea onChange={(e) => setNoteDraft(e.target.value)} />
            </div>
          )}
        </div>
        ...
        <div className={style.noteListWrapper}>
          {genNotes.map((note) => (
            <div key={note.noteId} className={style.noteDiv}>
              <h2 onClick={() => displayNoteHandler(note.noteId)}>
                {note.noteName}
              </h2>
              {displayNote === note.noteId && (
                <>
                  <p className={style.note}>{note.noteText}</p>
                  <button onClick={() => delNoteHandler(note.noteId)}>
                    Delete Note
                  </button>
                </>
              )}
